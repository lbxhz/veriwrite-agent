...
def render_pdf_acquisition_console(
    ...
    default_dois = [item.doi for item in selection.selected[: min(8, len(records))]]
    previous_dois = [
        doi
        for doi in st.session_state.get("v03_core_dois", default_dois)
        if doi in records
    ]
    selected_dois = st.multiselect(
        ...
        help="默认取排序靠前的 8 篇；非核心论文暂时只保留已验证元数据。",
    )
    ...
    if st.button(
        ...
        st.session_state["v03_pdf_inspection_json"] = batch.model_dump_json(
            indent=2
        )
        st.session_state.pop("v03_evidence_library_json", None)
        st.session_state.pop("v03_writing_handoff_json", None)
    ...
    _render_evidence_pipeline(selection, batch)
...
def _render_pdf_results(batch: PdfInspectionBatch) -> None:
    ...
    if counts["missing"]:
        st.warning(
            "缺失项会保留在队列中；下载后重新扫描即可继续，不需要重跑 V0.1/V0.2。"
        )
...
def _render_evidence_pipeline(
    ...
    target_words = _target_words(confirmed_requirement)
    ...
    if blockers:
        st.error(
            "当前V0.3仍是草案，不能交给V0.4。请补充缺失核心PDF、OCR或证据卡。"
        )
    ...
    if st.button(
        ...
        st.session_state["v03_writing_handoff_json"] = handoff.model_dump_json(
            indent=2
        )
        st.rerun()
...
def _build_evidence_library(
    ...
    def authority_fields(doi: str) -> tuple[list[str], str | None, str | None, str]:
        ...
        metadata = (
            ...
            if verification.authority is not None
            else None
        )
        return (
            ...
            (
                ...
                and verification.resolution.final_url
                else f"https://doi.org/{quote(doi, safe='/')}"
            ),
        )

    authority_by_doi = {
        item.doi: authority_fields(item.doi) for item in selection.selected
    }
    records = [
        LiteratureLibraryRecord(
            doi=item.doi,
            title=item.title,
            authors=authority_by_doi[item.doi][0],
            year=item.year,
            journal=authority_by_doi[item.doi][1],
            abstract=authority_by_doi[item.doi][2],
            source_url=authority_by_doi[item.doi][3],
            theme_ids=[item.theme_id],
            evidence_tier=("A_core" if item.doi in available else (
                "B_supporting" if item.doi in core_dois else "C_background"
            )),
            evidence_status=(
                "full_text_verified"
    ...
    pages = []
    cards = []
    card_extractor = LLMEvidenceCardExtractor(
    ...
    for document in available.values():
        ...
        if extraction.status != "complete":
            unresolved.append(
                f"pdf_extraction_{extraction.status}:{document.doi}"
            )
        ...
        try:
            ...
            unresolved.append(
                f"evidence_extraction_failed:{document.doi}:{exc}"
            )
...
def _target_words(confirmed: ConfirmedRequirementSpec) -> int:
    ...
    return (
        length.target_words
        or length.maximum_words
        or length.minimum_words
        or 4000
    )
